...
def utility_raw(pois: list[POI]) -> dict[str, float]:
    return {p.id: p.utility.raw for p in pois}


def utility_overall(pois: list[POI]) -> dict[str, float]:
    return {p.id: p.utility.overall for p in pois}


def anchor_overall(pois: list[POI]) -> dict[str, float]:
...
def wiki_description(poi: POI) -> str:
    ...
    if isinstance(enrichment, dict):
        return enrichment.description or ""
    return ""
...
def group_by_cluster(pois: list[POI]) -> dict[int, list[POI]]:
    grouped: dict[int, list[POI]] = defaultdict(list)
    for poi in pois:
        grouped[poi.cluster_id].append(poi)
    return dict(grouped)

def sort_by_utility(pois: list[POI]) -> list[POI]:
    return sorted(
        pois,
